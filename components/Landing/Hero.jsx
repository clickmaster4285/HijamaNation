...
export default function Hero() {
  ...
  return (
    <section
      className="relative flex items-start pb-12 md:pb-20 overflow-hidden pt-36 md:pt-44 lg:pt-52 xl:pt-60"
    >
      ...
      <div className="relative z-10 pl-8 pr-0 md:pl-16 lg:pl-20 max-w-4xl">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <p className="text-white/70 text-sm md:text-base tracking-widest font-light">
            UK's Leading Hijama Training Platform
          </p>
        </div>

        {/* Headline – home page content with line breaks & emphasis */}
        ...
        {/* Tagline – home page sub */}
        <p className="text-white text-lg md:text-xl italic font-light mb-5 tracking-wide">
          Learn Hijama. Book Treatment. Join a Trusted Community.
        </p>

        {/* Description – home page body */}
        <p className="text-white/70 text-sm md:text-base leading-relaxed mb-10 max-w-2xl">
          Hijama Nation is a UK-based platform for people who want to learn Hijama, book cupping therapy,
          or connect with certified practitioners with clear guidance, safe practice, and trusted support.
        </p>

        {/* Stats Row – unchanged (already matches home page stats) */}
        <div className="flex flex-wrap gap-x-6 gap-y-2">
          {stats.map((s) => (
            <div key={s.label} className="flex items-center gap-2">
              <span className="text-green-400 text-xs md:text-sm">⊙</span>
              <span className="text-white/70 text-sm md:text-base">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
